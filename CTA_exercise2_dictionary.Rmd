...
```
The results are broadly consistent with the interpretation provided in the tutorial. The Figure shows differences in overall tweet sentiment across newspapers. Specifically, tabloid newspapers such as The Daily Mirror, The Sun and The Daily Mail tend to produce more negative tweets overall compared to more traditional newspapers. Among tabloids, The Daily Mirror appears particularly negative. Interestingly, left-leaning newspapers such as The Daily Mirror and The Guardian also appear more negative within their respective categories.
...
tidy_dfm_news_lsd %>%
  ...
  theme_minimal()
```
The Figure presents sentiment trends over time for each of the eight British newspapers. Overall, sentiment varies across newspapers, but some common patterns can be observed. For several outlets, such as The Sun, Daily Mirror, and Daily Mail, sentiment appears to become slightly less negative over time, particularly towards April and May. In contrast, newspapers such as The Times and Metro show relatively stable sentiment throughout the period, with less pronounced fluctuations. These differences suggest that sentiment dynamics are not uniform across news sources, and that aggregating all newspapers together may obscure important source-specific trends.
...
```
Here, we manually construct a minimal dictionary of COVID-19–related terms.
The aim is to capture words that are salient and low-ambiguity in the context of UK news coverage during the early stages of the pandemic.
This dictionary is intentionally simple and is used to demonstrate a dictionary-based filtering technique rather than to make strong substantive claims.
...
totals_newspaper <- tidy_tweets %>%
  ...
  summarise(sum_words = sum(obs))
```
This step calculates the total number of words published by each newspaper.
Because the analysis is conducted at the word level, using word counts rather than tweet counts ensures that the resulting proportions are meaningful and comparable across outlets with different tweeting volumes.

```{r}
tidy_tweets %>%
  mutate(obs = 1) %>%
  filter(grepl(paste0(covid_words, collapse = "|"), 
               word, 
               ignore.case = TRUE)) %>%
  group_by(newspaper) %>%
  summarise(sum_covidwords = sum(obs)) %>%
  full_join(totals_newspaper, by = "newspaper") %>%
  mutate(
    sum_covidwords = ifelse(is.na(sum_covidwords), 0, sum_covidwords),
    pct_covidwords = sum_covidwords / sum_words
  ) %>%
  ggplot(aes(x = reorder(newspaper, -pct_covidwords),
             y = pct_covidwords)) +
  geom_point() +
  coord_flip() +
  xlab("Newspaper") +
  ylab("% of COVID-related words") +
  theme_minimal()
```

This figure shows the proportion of COVID-related words used in tweets by different British newspapers. Overall, there is some variation across outlets. The Telegraph and The Guardian show the highest relative use of COVID-related language, suggesting that pandemic-related issues featured more prominently in their Twitter coverage. By contrast, tabloids such as The Sun and The Times display a lower proportion of COVID-related terms.
To produce this figure, the tokenised tweets were filtered using a custom dictionary of COVID-related keywords. The number of matching words was then counted for each newspaper and normalised by the total number of words published by that outlet, allowing for comparison across newspapers with different volumes of tweets.
Although the absolute differences between newspapers are relatively small, this is expected. The analysis is conducted at the word level rather than the tweet level, and COVID-19 dominated news coverage across all outlets during this period. As a result, most newspapers refer to the pandemic to some extent. This simple dictionary-based approach is therefore useful for capturing relative thematic emphasis, but the results should be interpreted with caution, as they depend on the specific set of keywords included in the dictionary.
...
```
```{r}
totals_newspaper_q <- toks_news %>%
  dfm() %>%
  dfm_group(groups = username) %>%
  convert(to = "data.frame") %>%
  mutate(
    sum_words = rowSums(across(where(is.numeric)))
  ) %>%
  select(username = doc_id, sum_words)
```

```{r}
# Create a dictionary with two categories:
# one for general COVID terms, one for health system / severity terms
covid_dict <- dictionary(list(
  covid_general = c("covid", "coronavirus", "pandemic", "lockdown", "virus"),
  covid_health  = c("hospital", "death", "cases", "icu", "nhs", "patients")
))
```
This step we manually construct a small dictionary with two thematic categories. This allows us to distinguish between general pandemic references and health-related severity. The choice of words reflects substantive judgement rather than automation.

```{r}
dfm_covid <- toks_news %>%
  tokens_lookup(dictionary = covid_dict) %>%
  dfm() %>%
  dfm_group(groups = username)
```
1.tokens_lookup() identifies dictionary terms in the tokenised text
2.dfm() creates a document-feature matrix
3.Grouping by username aggregates all tweets from the same newspaper

In order to group dictionary-based word counts by newspaper, it is necessary to preserve document-level metadata during tokenisation. We therefore first construct a quanteda corpus using the tweet text and associated variables, and then generate tokens from this corpus.

This approach ensures that document variables such as username remain available as docvars, allowing us to aggregate dictionary matches by newspaper using dfm_group(). Tokenising the raw text directly would discard this metadata and prevent grouping at the source level.

```{r}
tidy_covid_dfm <- dfm_covid %>%
  convert(to = "data.frame") %>%
  rename(username = doc_id) %>%
  mutate(
    covid_general = ifelse(is.na(covid_general), 0, covid_general),
    covid_health  = ifelse(is.na(covid_health), 0, covid_health)
  )
```
Converting the dfm into a data frame makes it easier to inspect and visualise and renaming variables improves interpretability.
Replacing missing values with zeros ensures that newspapers with no matches are treated as having zero COVID-related words rather than missing data.

```{r}
covid_prop <- tidy_covid_dfm %>%
  left_join(totals_newspaper_q, by = "username") %>%
  mutate(
    covid_total = covid_general + covid_health,
    pct_covid_words = covid_total / sum_words
  )
```
Normalising by the total number of words controls for differences in output volume. This allows us to compare relative emphasis rather than absolute frequency. We explicitly handle missing or zero denominators to avoid undefined proportions that would otherwise remove observations during plotting.

```{r}
covid_prop %>%
  ggplot(aes(
    x = reorder(username, pct_covid_words),
    y = pct_covid_words
  )) +
  geom_point() +
  coord_flip() +
  ...
  theme_minimal()
```
This figure shows the proportion of COVID-related words across tweets published by eight British newspapers. Although the overall differences are relatively modest, clear variation can still be observed. The Guardian, the Daily Mirror, and the Daily Mail show the highest relative use of COVID-related language, suggesting that pandemic-related issues were more prominent in their Twitter coverage during this period.
By contrast, outlets such as The Times and The Sun display lower proportions of COVID-related terms. Importantly, these results are based on word-level proportions rather than tweet counts. This means the analysis captures the relative emphasis placed on COVID-19 within each newspaper’s overall output, rather than absolute volume. As a result, the differences should be interpreted cautiously and are dependent on the specific set of keywords included in the dictionary.

3. Apply the Lexicoder Sentiment Dictionary to the news tweets, but break down the analysis by newspaper
```{r}
# Create a document-feature matrix from Lexicoder-annotated tokens
# and group by newspaper (username)
dfm_news_lsd <- dfm(toks_news_lsd) %>% 
  dfm_group(groups = username)
```
We construct a document-feature matrix from the Lexicoder-annotated tokens and aggregate the counts by newspaper. This allows us to compare overall sentiment across news outlets.
```{r}
tidy_dfm_news_lsd <- dfm_news_lsd %>%
  convert(to = "data.frame") %>%
  ...
  mutate(sentiment = positive - negative)
```
The dfm is converted into a data frame for easier inspection and plotting. We calculate net sentiment by subtracting negative from positive word counts.
```{r}
tidy_dfm_news_lsd %>%
  ggplot() +
  geom_point(aes(x = reorder(newspaper, -sentiment),
                 y = sentiment)) +
  coord_flip() +
  xlab("Newspaper") +
  ...
  theme_minimal()
```
This figure shows overall tweet sentiment across eight British newspapers using the Lexicoder Sentiment Dictionary. Sentiment is calculated at the word level and aggregated by newspaper, with net sentiment defined as the difference between positive and negative word counts.
All newspapers display negative overall sentiment, which is expected given that news reporting—particularly during the early stages of the COVID-19 pandemic—contains a high prevalence of negative or problem-oriented language. However, clear differences can still be observed across outlets.
Tabloid newspapers such as Daily Mirror, The Sun, and Daily Mail UK show more negative overall sentiment, while newspapers such as The Guardian, The Times, and The Telegraph appear relatively less negative. This suggests that affective tone varies systematically across news sources.
While the Lexicoder dictionary is well suited for analysing news text, these results should be interpreted with caution, as the analysis is based on aggregate word frequencies and does not account for context or framing within individual tweets.
